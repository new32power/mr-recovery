...
export function MasterLogin({ onAuth }: { onAuth: (token: string, sessionId: string) => void }) {
  ...
  // Fetch GOOGLE_CLIENT_ID from runtime API
  useEffect(() => {
    apiFetch("/api/auth/config")
      .then(r => r.json() as Promise<{ googleClientId?: string | null }>)
  ...
  useEffect(() => {
    ...
    script.onerror = () => setErr("Google script load failed. Check internet connection.");
    document.head.appendChild(script);
    return () => { try { document.head.removeChild(script); } catch { /* ignore */ } };
  }, [googleClientId]);

  // Initialize GSI + render official Google button (works on mobile & desktop)
  useEffect(() => {
    if (!gsiReady || !googleClientId || !btnRef.current) return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const handleCredential = async (response: { credential: string }) => {
      ...
      try {
        const r = await apiFetch("/api/auth/google-verify", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ credential: response.credential }),
        });
        if (!r.ok) { const j = await r.json() as { error?: string }; setErr(j.error ?? "Access denied. Wrong Google account."); return; }
    ...
    g.accounts.id.initialize({ client_id: googleClientId, callback: handleCredential });

    // renderButton — official Google button, works on ALL devices including mobile
  ...
  }, [gsiReady, googleClientId, onAuth]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(ellipse at 65% 15%, rgba(99,102,241,0.14) 0%, transparent 55%), radial-gradient(ellipse at 15% 85%, rgba(139,92,246,0.10) 0%, transparent 50%), ${T.bg}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter', system-ui, sans-serif", padding: "20px" }}>
      <div style={{ width: "100%", maxWidth: 400, background: T.card, borderRadius: 24, padding: "44px 36px 36px", border: `1px solid ${T.borderLight}`, boxShadow: "0 32px 80px rgba(0,0,0,.7)" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          ...
          <div style={{ fontSize: 22, fontWeight: 900, color: T.text, letterSpacing: 1 }}>MR ROBOT</div>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginTop: 10, fontSize: 10, color: T.accentLight, background: "rgba(99,102,241,0.12)", padding: "4px 16px", borderRadius: 99, border: "1px solid rgba(99,102,241,0.25)", fontWeight: 700, letterSpacing: 2, textTransform: "uppercase" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
          {!configLoaded ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8, color: T.muted, fontSize: 13, height: 44 }}>
              <Spinner /> Loading…
            </div>
          ) : googleClientId ? (
            <>
              <div style={{ fontSize: 13, color: T.muted, textAlign: "center" }}>Apne authorized Gmail account se sign in karo</div>
              {/* Official Google button — renderButton() handles mobile + desktop */}
              {loading ? (
                <div style={{ display: "flex", alignItems: "center", gap: 8, color: T.muted, fontSize: 13, height: 44 }}><Spinner /> Verifying…</div>
              ) : !gsiReady ? (
                <div style={{ display: "flex", alignItems: "center", gap: 8, color: T.muted, fontSize: 13, height: 44 }}><Spinner /> Loading Google…</div>
              ) : null}
              <div ref={btnRef} style={{ width: "100%", minHeight: 44, display: "flex", justifyContent: "center" }} />
            </>
          ) : (
            <div style={{ width: "100%", padding: "18px 16px", borderRadius: 12, background: "rgba(245,158,11,0.07)", border: "1px solid rgba(245,158,11,0.25)", textAlign: "center" }}>
              <div style={{ fontSize: 24, marginBottom: 8 }}>⚙️</div>
              <div style={{ fontSize: 13, color: "#f59e0b", fontWeight: 700, marginBottom: 6 }}>Google Sign-In Setup Required</div>
              <div style={{ fontSize: 11, color: T.muted, lineHeight: 1.6 }}>
                Cloudflare dashboard mein<br />
                <code style={{ background: T.border, padding: "2px 7px", borderRadius: 4, fontSize: 10, color: T.text }}>GOOGLE_CLIENT_ID</code> set karo
              </div>
            </div>
          )}
          {loading && <div style={{ display: "flex", alignItems: "center", gap: 8, color: T.muted, fontSize: 13 }}><Spinner /> Verifying…</div>}
          {err && <div style={{ width: "100%" }}><ErrBanner msg={err} /></div>}
        </div>

        <div style={{ textAlign: "center", marginTop: 32, paddingTop: 16, borderTop: `1px solid ${T.border}`, fontSize: 11, color: T.muted }}>
  ...
  function LoginLimitModal({ masterPin, onClose }: { masterPin: string; onClose: () => void }) {
    ...
    return (
      <Modal onClose={onClose} maxWidth={380}>
        <ModalHeader title="Master Login Limit" icon={<Ic.Lock />} onClose={onClose} />
        {loading ? <div style={{ padding: 20, textAlign: "center", color: T.muted, fontSize: 13 }}>Loading…</div> : (
          <form onSubmit={handleSubmit}>
            ...
            <button type="submit" disabled={saving} style={{ width: "100%", padding: "10px 0", borderRadius: 9, border: "none", background: T.accent, color: "#fff", fontWeight: 700, fontSize: 13, cursor: saving ? "default" : "pointer", opacity: saving ? 0.7 : 1 }}>{saving ? "Saving…" : "Save"}</button>
          </form>
        )}
      </Modal>
    );
...
function Dashboard({ masterPin, sessionId, onLogout, onPinChanged, onSessionIdUpdate }: { masterPin: string; sessionId: string; onLogout: () => void; onPinChanged: (p: string) => void; onSessionIdUpdate: (id: string) => void }) {
  ...
  const [showLoginLimit, setShowLoginLimit] = useState(false);
    ...
    <div style={{ minHeight: "100vh", background: T.bg, fontFamily: "'Inter', system-ui, sans-serif", color: T.text }}>
      ...
      <div style={{ background: T.headerBg, borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 50, backdropFilter: "blur(20px)" }}>
        ...
        <div style={{ maxWidth: 960, margin: "0 auto", display: "flex", alignItems: "center", gap: 8, padding: "0 12px", height: 50, flexWrap: "nowrap", overflowX: "auto", scrollbarWidth: "none" }}>
          ...
          <button onClick={() => setShowLoginLimit(true)} title="Master Login Limit" style={{ width: 32, height: 32, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", border: `1px solid ${T.borderLight}`, background: T.card, color: T.muted, flexShrink: 0 }}><Ic.Lock /></button>
      ...
      {showLoginLimit && <LoginLimitModal masterPin={masterPin} onClose={() => setShowLoginLimit(false)} />}
