...
function SettingsPage({ appId, isDark, onToggleDark, devices, onLogout, msgCount, isZeroTrace: isZT, onDeleteProtEnabledChange, panelToken, onPanelTokenChange }: {
  ...
  const [cpLoading, setCpLoading] = useState(false);
    ...
    /* ── Login Limit ── */
    const [llValue, setLlValue] = useState<number | null>(null);
    ...
    useEffect(() => {
      apiFetch(`/api/apps/${appId}`)
        .then(r => r.json())
        .then((d: { loginLimit?: number }) => { if (typeof d.loginLimit === "number") { setLlValue(d.loginLimit); setLlInput(String(d.loginLimit)); } })
        .catch(() => {});
    }, [appId]);
    async function handleSaveLoginLimit() {
      const n = Number(llInput);
      if (!Number.isFinite(n) || n < 1) { setLlMsg("Enter a valid number (1 or more)."); return; }
      setLlSaving(true); setLlMsg("");
      try {
        const r = await apiFetch(`/api/apps/${appId}`, {
          method: "PATCH", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ loginLimit: Math.floor(n) }),
        });
        const j = await r.json().catch(() => ({})) as { error?: string; loginLimit?: number };
        if (!r.ok) { setLlMsg(j.error || "Failed. Try again."); return; }
        setLlValue(Math.floor(n));
        setLlMsg("Saved!");
        setTimeout(() => setLlMsg(""), 2000);
      } catch { setLlMsg("Network error. Try again."); }
      ...
      <div style={{ background: t.card, borderRadius: 10, border: `1px solid ${t.cardB}`, overflow: "hidden" }}>
        ...
        {sessions.length === 0 ? (
            ...
            sessions.map((s, idx) => {
              ...
              return (
                <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: isMine ? (t === DT ? "#2e1f5e" : "#f5f3ff") : "transparent", borderTop: idx > 0 ? `1px solid ${t.hdrB}` : "none" }}>
                  <div style={{ width: 36, height: 36, borderRadius: 9, background: isMine ? t.accent : t.muted, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 15 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                      ...
                      {isMine && <span style={{ background: t.accent, color: "#fff", borderRadius: 99, padding: "1px 6px", fontSize: 9, fontWeight: 800 }}>THIS DEVICE</span>}
                    </div>
                    <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 1 }}>Login: {fmtTime(s.loginTime)} · IP: {s.ip || "unknown"} · Last active: {fmtTime(s.lastActive)}</div>
                  </div>
      ...
      <div style={{ background: t.card, borderRadius: 10, border: `1px solid ${t.cardB}`, padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={() => { setCpOpen(true); setCpCurrent(""); setCpNew(""); setCpNew2(""); setCpErr(""); setCpMsg(""); }} style={{ padding: "7px 14px", borderRadius: 8, background: t.accent, border: "none", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
          Change
            ...
            {/* ── Login Limit ── */}
        <div style={{ background: t.card, borderRadius: 10, border: `1px solid ${t.cardB}`, padding: "12px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={t.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
            </svg>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: t.txt }}>Login Limit</div>
              <div style={{ fontSize: 11, color: t.muted, marginTop: 1 }}>Max devices that can be logged in at once{llValue !== null ? ` (current: ${llValue})` : ""}</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              type="number" min={1} value={llInput} onChange={e => { setLlInput(e.target.value); setLlMsg(""); }}
              style={{ width: 90, padding: "8px 10px", borderRadius: 8, border: `1.5px solid ${t.cardB}`, background: t.hdr, color: t.txt, fontSize: 13, outline: "none" }}
            />
            <button onClick={handleSaveLoginLimit} disabled={llSaving} style={{ padding: "7px 14px", borderRadius: 8, background: t.accent, border: "none", color: "#fff", fontSize: 12, fontWeight: 700, cursor: llSaving ? "not-allowed" : "pointer", opacity: llSaving ? 0.6 : 1 }}>
              {llSaving ? "Saving..." : "Save"}
            </button>
            {llMsg && <span style={{ fontSize: 12, fontWeight: 600, color: llMsg === "Saved!" ? "#4ade80" : "#ef4444" }}>{llMsg}</span>}
          </div>
        </div>
...
export default function WebDashboard() {
  ...
  useEffect(() => {
    ...
    async function pingSession() {
      const sid = localStorage.getItem(`mrrobot_session_id_${appId}`);
      if (!sid) return; // No session to ping — JWT alone handles auth
      try {
